function params = PBDL_core_pairs_standard_form(y, X, lambda)
...
A_eps_iota = sparse(I,J,V,n*(n-1), n*(n-1));
...
A1 = [sparse(n*(n-1),n*(n-1)), A_v, A_g, -A_g, sparse(n*(n-1),1), A_eps,sparse(n*(n-1),n*(n-1))];
A2 = [sparse(n,n+n*(n-1)),A_norm, A_norm, -ones(n,1), sparse(n,n*(n-1)),sparse(n,n*(n-1))];
% A3 = [-A_eps, A_v_l, A_g_l, -A_g_l, sparse(n*(n-1),1), sparse(n*(n-1),n*(n-1))];
A3 = [-A_eps, sparse(n*(n-1),n),  sparse(n*(n-1),n*d), sparse(n*(n-1),n*d), ...
    sparse(n*(n-1),1), A_eps_iota, A_eps ];
    

%% cost vector;
c = zeros(1, n + 3*n*(n-1) + 2*n*d + 1);
c(1:n*(n-1)) = 1/(n*(n-1));
c(n + n*(n-1) + 2*n*d + 1) = 1/sqrt(n*(n-1))*lambda;

%% solution bounds
lb = zeros(n + 3*n*(n-1) + 2*n*d + 1, 1);
