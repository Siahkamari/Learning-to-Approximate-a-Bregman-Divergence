function params = NBDLLinf(X_train, S1, S2, S3, S4, lambda)
...
for i=1:n_S
    ...
    count = count + 1;
    A1(count,:) = -[a,b,0,zeros(1,m)];
    end
end
b1 = zeros(n_S*(K-1),1);

% supervision inequalities
A3 = zeros([m, K*(dim+1)+1+m]);
for count=1:m
    ...
    A3(count,:) = a1 - a2;
    A3(count,K*(dim+1)+1+count) = -1;
end
b3 = -1*ones(m,1);

% condition -L < a_ij < L
A4 = [zeros(K*dim,K), -eye(K*dim),-ones(K*dim,1),zeros(K*dim,m)];
A5 = [zeros(K*dim,K), eye(K*dim),-ones(K*dim,1),zeros(K*dim,m)];
b4 = zeros(K*dim,1);
b5 = zeros(K*dim,1);

% condition that eps_i  > 0
A6 = [zeros(m,K*(dim+1)+1), -eye(m)];
b6 = zeros(m,1);

% price vectors
C1 = [zeros(1,K*(dim+1)),0,ones(1,m)];  
C2 = [zeros(1,K*(dim+1)),1,zeros(1,m)];                         % sum (t_i)                       % sum (eps_i)
C = C1/m + lambda*C2;
