function [acc, pred] = Copy_of_cross_validate_knn(y, X, tCL, num_folds, knn_size, method)
...
for i=1:num_folds
    ...
    elseif method == "ITML"
        ...
        divs = mahalanobis(X_test, X_train, M);
    elseif method == "NBDL_means"
        max_y = max(y_train);
        params = cell(max_y,1);
        X_bar = zeros(max_y,dim);
        divs = zeros(length(y_test),max_y);
        for class_k=1:max_y
            params{class_k} = NBDL_k_mean(y_train, X_train, class_k);
            X_bar(class_k,:) = mean(X_train(y_train==class_k, :));
            divs(:,class_k) = max_affine_bregman(X_test, X_bar(i,:), params{class_k});
        end
        
        [~, pred(test_start:test_end)] = min(divs,[],2);
       
    elseif method == "NBDL"
        params = NBDL(y_train, X_train);
